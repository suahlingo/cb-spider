...
type AzureCustomDiskHandler struct {
...
func (diskHandler *AzureCustomDiskHandler) CustomCreateDisk(DiskReqInfo irs.CustomDiskInfo) (diskInfo irs.CustomDiskInfo, createErr error) {
	hiscallInfo := GetCallLogScheme(diskHandler.Region, call.CUSTOMDISK, DiskReqInfo.IId.NameId, "CreateDisk()")
	start := call.Start()
	...
	if err != nil {
		...
		return irs.CustomDiskInfo{}, createErr
	}
	...
	if err != nil {
		...
		return irs.CustomDiskInfo{}, getErr
	}
	info, err := CustomSetterDiskInfo(&disk)
	if err != nil {
		getErr := errors.New(fmt.Sprintf("Failed to Get Disk. err = %s", err.Error()))
		cblogger.Error(getErr.Error())
		LoggingError(hiscallInfo, getErr)
...
func (diskHandler *AzureCustomDiskHandler) CustomListDisk() ([]*irs.CustomDiskInfo, error) {
	hiscallInfo := GetCallLogScheme(diskHandler.Region, call.CUSTOMDISK, "DISK", "ListDisk()")
	start := call.Start()
	...
	var diskStatusList []*irs.CustomDiskInfo
	for _, disk := range diskList {
		diskStatus, err := CustomSetterDiskInfo(disk)
		if err != nil {
			getErr := errors.New(fmt.Sprintf("Failed to List Disk. err = %s", err.Error()))
			cblogger.Error(getErr.Error())
			LoggingError(hiscallInfo, getErr)
...
func (diskHandler *AzureCustomDiskHandler) CustomGetDisk(diskIID irs.IID) (irs.CustomDiskInfo, error) {
	hiscallInfo := GetCallLogScheme(diskHandler.Region, call.CUSTOMDISK, diskIID.NameId, "GetDisk()")
	start := call.Start()
...
func (diskHandler *AzureCustomDiskHandler) CustomChangeDiskSize(diskIID irs.IID, size string) (bool, error) {
	hiscallInfo := GetCallLogScheme(diskHandler.Region, call.CUSTOMDISK, diskIID.NameId, "ChangeDiskSize()")
	start := call.Start()
	// Exist Disk
...
func (diskHandler *AzureCustomDiskHandler) CustomDeleteDisk(diskIID irs.IID) (bool, error) {
	hiscallInfo := GetCallLogScheme(diskHandler.Region, call.CUSTOMDISK, diskIID.NameId, "DeleteDisk()")
	start := call.Start()
	convertedDiskIId, err := ConvertDiskIID(diskIID, diskHandler.CredentialInfo, diskHandler.Region)
	if err != nil {
...
func (diskHandler *AzureCustomDiskHandler) CustomAttachDisk(diskIID irs.IID, ownerVM irs.IID) (irs.CustomDiskInfo, error) {
	hiscallInfo := GetCallLogScheme(diskHandler.Region, call.CUSTOMDISK, diskIID.NameId, "AttachDisk()")
	start := call.Start()
	...
	if err != nil {
		...
		return irs.CustomDiskInfo{}, attachErr
	}
	info, err := CustomSetterDiskInfo(&disk)
	if err != nil {
		attachErr := errors.New(fmt.Sprintf("Failed to AttachDisk. err = %s", err.Error()))
...
func (diskHandler *AzureCustomDiskHandler) CustomDetachDisk(diskIID irs.IID, ownerVM irs.IID) (bool, error) {
	hiscallInfo := GetCallLogScheme(diskHandler.Region, call.CUSTOMDISK, diskIID.NameId, "DetachDisk()")
	start := call.Start()
	convertedDiskIId, err := ConvertDiskIID(diskIID, diskHandler.CredentialInfo, diskHandler.Region)
	if err != nil {
...
func CustomSetterDiskInfo(disk *armcompute.Disk) (*irs.CustomDiskInfo, error) {
	diskStatus := irs.CustomDiskInfo{
		IId: irs.IID{
			NameId:   *disk.Name,
			SystemId: *disk.ID,
		},
	...
	if disk.Properties != nil {
		...
		if disk.Properties.ProvisioningState != nil {
			...
			case "Creating":
				diskStatus.Status = irs.CustomDiskCreating
			case "Deleting":
				diskStatus.Status = irs.CustomDiskDeleting
			case "Failed":
				diskStatus.Status = irs.CustomDiskError
			case "Migrating", "Updating":
				diskStatus.Status = irs.CustomDiskAttached
			case "Succeeded":
				if *disk.Properties.DiskState == armcompute.DiskStateUnattached {
					diskStatus.Status = irs.CustomDiskAvailable
				} else {
					diskStatus.Status = irs.CustomDiskAttached
				}
			}
...
func (diskHandler *AzureCustomDiskHandler) validationDiskReq(diskReq irs.CustomDiskInfo) error {
...
func CustomGetRawDisk(diskIID irs.IID, resourceGroup string, client *armcompute.DisksClient, ctx context.Context) (armcompute.Disk, error) {
	if diskIID.NameId == "" {
		var CustomdiskList []*armcompute.Disk
		...
		for pager.More() {
			...
			for _, disk := range page.Value {
				CustomdiskList = append(CustomdiskList, disk)
			}
		}

		for _, disk := range CustomdiskList {
...
func CustomConvertDiskIID(diskIID irs.IID, credentialInfo idrv.CredentialInfo, regionInfo idrv.RegionInfo) (irs.IID, error) {
	if diskIID.NameId == "" && diskIID.SystemId == "" {
		return diskIID, errors.New(fmt.Sprintf("invalid IID"))
	}
	if diskIID.SystemId == "" {
...
func CustomCheckExistDisk(diskIID irs.IID, resourceGroup string, client *armcompute.DisksClient, ctx context.Context) (bool, error) {
	var diskList []*armcompute.Disk
...
func CustomgetMinDataDiskLun(dataDisks []*armcompute.DataDisk) (int32, error) {
	if len(dataDisks) == 0 {
		return int32(0), nil
	}
...
func CustomcheckSize(newSize string, oldSize int32) (int32, error) {
...
func CustomcheckChangeStatus(disk armcompute.Disk) error {
...
func CustomcheckDeleteStatus(disk armcompute.Disk) error {
	if disk.Properties != nil && disk.Properties.ProvisioningState != nil && disk.Properties.DiskState != nil {
		if *disk.Properties.ProvisioningState == "Succeeded" && (*disk.Properties.DiskState == armcompute.DiskStateUnattached || *disk.Properties.DiskState == armcompute.DiskStateReserved) {
			return nil
		}
...
func CustomCheckAttachStatus(disk *armcompute.Disk) error {
	if disk.Properties != nil && disk.Properties.ProvisioningState != nil && disk.Properties.DiskState != nil {
...
func CustomAttach(diskIID irs.IID, ownerVM irs.IID, credentialInfo idrv.CredentialInfo, region idrv.RegionInfo, ctx context.Context, vmClient *armcompute.VirtualMachinesClient, diskClient *armcompute.DisksClient) (armcompute.Disk, error) {
...
func CustomAttachList(diskIIDList []irs.IID, ownerVM irs.IID, credentialInfo idrv.CredentialInfo, region idrv.RegionInfo, ctx context.Context, vmClient *armcompute.VirtualMachinesClient, diskClient *armcompute.DisksClient) (armcompute.VirtualMachine, error) {
