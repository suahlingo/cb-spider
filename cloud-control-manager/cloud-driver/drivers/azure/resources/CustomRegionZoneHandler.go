...
type AzureCustomRegionZoneHandler struct {
...
func (regionZoneHandler *AzureCustomRegionZoneHandler) CustomListRegionZone() ([]*irs.CustomRegionZoneInfo, error) {
	hiscallInfo := GetCallLogScheme(regionZoneHandler.Region, call.CUSTOMREGIONZONE, "RegionZone", "ListRegionZone()")
	start := call.Start()
	...
	for pager.More() {
		...
		if err != nil {
			...
			return []*irs.CustomRegionZoneInfo{}, getErr
		}
	...
	var regionZoneInfo []*irs.CustomRegionZoneInfo
	...
	for i := 0; i < lenLocations; {
		...
		for j := 0; j < routineMax; j++ {
			go func(wait *sync.WaitGroup, loc *armsubscription.Location) {
				...
				mutex.Lock()
				regionZoneInfo = append(regionZoneInfo, &irs.CustomRegionZoneInfo{
					Name:         *loc.Name,
...
func (regionZoneHandler *AzureCustomRegionZoneHandler) CustomGetRegionZone(Name string) (irs.CustomRegionZoneInfo, error) {
	hiscallInfo := GetCallLogScheme(regionZoneHandler.Region, call.CUSTOMREGIONZONE, "RegionZone", "GetRegionZone()")
	start := call.Start()
...
func (regionZoneHandler *AzureCustomRegionZoneHandler) CustomListOrgRegion() (string, error) {
	hiscallInfo := GetCallLogScheme(regionZoneHandler.Region, call.CUSTOMREGIONZONE, "RegionZone", "ListOrgRegion()")
	start := call.Start()
...
func (regionZoneHandler *AzureCustomRegionZoneHandler) CustomListOrgZone() (string, error) {
	hiscallInfo := GetCallLogScheme(regionZoneHandler.Region, call.CUSTOMREGIONZONE, "RegionZone", "ListOrgZone()")
	start := call.Start()
